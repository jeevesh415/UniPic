...
if __name__ == '__main__':
    ...
    parser.add_argument('--lr_fake_score', default=4e-7, type=float, help='learning rate for fake score.')
    ...
    parser.add_argument('--lr_scheduler', default='constant', type=str, help='learning rate scheduler.')
    ...
    parser.add_argument('--student', default='work_dirs/Qwen-Image-Edit-DMD-full-20k-bsz64-all-datasets/step10000/ema_transformer', type=str)
    # parser.add_argument('--student', default='/data_genie/genie/why/Qwen-Image-Dev-main/work_dirs/Qwen-Image-Edit-FSDP-6imgs-all-datasets-step100k-bsz16/step100000/transformer', type=str)
    ...
    # # LoRA 相关参数
    # parser.add_argument('--lora_r', default=512, type=int, help='LoRA rank')
    ...
    parser.add_argument('--fake_score_warmup_steps', default=0, type=int, help='lora warmup steps (optimizer updates).')
    ...
    transformer = QwenImageTransformer2DModel.from_pretrained(
        pretrained_model_name_or_path=args.student,
        torch_dtype=torch.bfloat16,
        device_map=None,
        low_cpu_mem_usage=True
    )
    ...
    fake_score = QwenImageTransformer2DModel.from_pretrained(
    ...
    params2dims = {k: v.dim() for k, v in fake_score.named_parameters()}
    decay_params = []
    nodecay_params = []
    for k, v in fake_score.named_parameters():
        k = k.replace('_fsdp_wrapped_module.', '')
        if params2dims[k] >= 2:
            decay_params.append(v)
        else:
    ...
    optimizer_fake_score = AdamW(params=optim_groups,
                      lr=args.lr,
                      betas=(args.beta1, args.beta2))
    ...
    for train_step in range(1, args.train_steps+1):
        ...
        if train_step % args.student_update_freq == 0 and train_step > args.fake_score_warmup_steps: 
            # ==================== Student Update ====================
            transformer.train()
            fake_score.eval()
            ...
            pred_original_samples = x_cur - timesteps_with_grad * model_pred
            ...
            with torch.no_grad(), torch.amp.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=True):
                ...
                teacher_pred_cond = forward_fn(pipeline, teacher, noisy_model_input, latents_list[1:], timesteps, prompt_embeds, prompt_embeds_mask, img_shapes_list)
                teacher_pred_uncond = forward_fn(pipeline, teacher, noisy_model_input, latents_list[1:], timesteps, null_embeds, null_embeds_mask, img_shapes_list)
                ...
                fake_pred = forward_fn(pipeline, fake_score, noisy_model_input, latents_list[1:], timesteps, prompt_embeds, prompt_embeds_mask, img_shapes_list)
                # fake_pred_uncond = forward_fn(pipeline, fake_score, noisy_model_input, latents_list[1:], timesteps, null_embeds, null_embeds_mask, img_shapes_list)
                # fake_pred = fake_pred_uncond + args.guidance_scale * (fake_pred_cond - fake_pred_uncond)

            with torch.no_grad():
                weight_factor =  (
                    torch.abs(pred_original_samples.double() - noises.double() + teacher_pred.double())
                    .mean(dim=[1, 2, 3, 4], keepdim=True)
                    .clip(min=0.00001)
                )
            score_gradient = torch.nan_to_num(
                (teacher_pred.double() - fake_pred.double()) / weight_factor
                , nan=0, posinf=1e5, neginf=-1e5
            )
            ...
            with sync_ctx:
                dmd_loss = torch.nan_to_num(dmd_loss, nan=0, posinf=1e5, neginf=-1e5)
                dmd_loss = dmd_loss / args.accum_steps
                dmd_loss.backward()
            if sync_now:
            ...
            teacher_pred = forward_fn(
                pipeline, fake_score, noisy_model_input, latents_list[1:], timesteps, 
                prompt_embeds, prompt_embeds_mask, img_shapes_list,
            )
            fake_score_loss = F.mse_loss(teacher_pred.float(), target.float()) 
            fake_score_loss = fake_score_loss + torch.mean(1 - F.cosine_similarity(teacher_pred.float(), target.float(), dim=1))
            fake_score_loss = torch.nan_to_num(fake_score_loss, nan=0, posinf=1e5, neginf=-1e5)

            fake_score_step += 1
            sync_now = fake_score_step % args.accum_steps == 0
            sync_ctx = nullcontext() if sync_now else fake_score.no_sync()
            with sync_ctx:
                scaled_fake_score_loss = fake_score_loss / args.accum_steps
                scaled_fake_score_loss.backward()
            if sync_now:
                grad_norm_fake_score = fake_score.clip_grad_norm_(args.grad_clip)
                optimizer_fake_score.step()
                lr_scheduler_fake_score.step()
        ...
        if update_log:
            ...
            if train_step > args.fake_score_warmup_steps + args.student_update_freq:
                logger.info(f"Train steps={train_step}/{args.train_steps}, Student_step={student_step}, Teacher_step={fake_score_step}, "
                        f"dmd_loss={dmd_loss.item():.4f}, lora_loss={fake_score_loss.item():.4f}, "
                        f"grad norm={grad_norm.item():.4f}, teacher grad norm={grad_norm_fake_score.item():.4f}, speed: {speed:.4f}, LR: {cur_lr:.8f}")
            else:
                logger.info(f"Train steps={train_step}/{args.train_steps}, Teacher_step={fake_score_step}, lora_loss={fake_score_loss.item():.4f}, "
                        f"teacher grad norm={grad_norm_fake_score.item():.4f}, speed: {speed:.4f}, LR: {cur_lr:.8f}")
            tik = time.time()
        ...
        if save_ckpt:
            ...
            if ddp_rank == 0:
                ...
                os.makedirs(os.path.join(save_dir, "transformer"), exist_ok=True)
                os.makedirs(os.path.join(save_dir, "ema_transformer"), exist_ok=True)

                torch.save(student_state, os.path.join(save_dir, "transformer", "diffusion_pytorch_model.bin"))
                torch.save(ema_state, os.path.join(save_dir, "ema_transformer", "diffusion_pytorch_model.bin"))
                transformer.module.to_json_file(os.path.join(save_dir, "transformer", "config.json"))
                ema_transformer.module.to_json_file(os.path.join(save_dir, "ema_transformer", "config.json"))
