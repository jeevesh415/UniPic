...
class QwenImageEditPipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    ...
    def prepare_latents(
        ...
        return latents_list, image_latents_list
    ...
    def __call__(
        ...
        # prepare multiple images for input
        total_number_of_pixels = sum([math.prod(image.size) for image in images])
        ratio = image_size / total_number_of_pixels ** 0.5
        ...
        # prepare multiple images for output
        total_number_of_pixels = sum(map(lambda w, h: w * h, widths, heights))
        ratio = image_size / total_number_of_pixels ** 0.5
        widths, heights = zip(*map(lambda w, h: (round(w * ratio), round(h * ratio)), widths, heights))


        multiple_of = self.vae_scale_factor * 2
        widths = [width // multiple_of * multiple_of for width in widths]
        heights = [height // multiple_of * multiple_of for height in heights]
        ...
        image_seq_len = [latent.shape[1] for latent in latents]
        mu = calculate_shift(
            sum(image_seq_len),
        ...
        latents = torch.split(latents, split_size_or_sections=image_seq_len, dim=1)

        images = []
        for latent, width, height in zip(latents, widths, heights):
            if output_type == "latent":
                ...
                latent_std = 1.0 / torch.tensor(self.vae.config.latents_std).view(1, self.vae.config.z_dim, 1, 1, 1).to(
                    latent.device, latent.dtype
                )
                latent = latent / latent_std + latent_mean
                image = self.vae.decode(latent, return_dict=False)[0][:, :, 0]
