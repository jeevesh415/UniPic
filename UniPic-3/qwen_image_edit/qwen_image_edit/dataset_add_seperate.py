...
class Unipic3EditDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            # 统一处理 output_image 字段：支持字符串（单图）或列表（多图）
            output_image_field = data_sample['output_image']
            if isinstance(output_image_field, str):
                # 单图：字符串格式
                output_images = [self._read_image(output_image_field).convert('RGB')]
            elif isinstance(output_image_field, list):
                # 多图：列表格式
                output_images = [self._read_image(output_image).convert('RGB')
                                 for output_image in output_image_field]
            else:
                raise ValueError(f"output_image must be str or list, got {type(output_image_field)}")
            output_images = self._resize_images(output_images)
...
class TryOnDataset(Unipic3EditDataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            # 统一返回 output_images 列表格式，与 Unipic3EditDataset 保持一致
            data = dict(
                input_images=input_images, output_images=[output_image],
                image_dir=self.image_folder,
                type='image2image', text=prompt)
