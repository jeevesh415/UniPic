...
from qwen_image_edit.pipeline_qwenimage_edit_add_seperate import calculate_shift, QwenImageEditPipeline
...
if __name__ == '__main__':
    ...
    # CHANGED: load transformer on CPU to avoid full model on a single GPU
    transformer = QwenImageTransformer2DModel.from_pretrained(
        pretrained_model_name_or_path=args.transformer, subfolder='transformer',
        torch_dtype=torch.bfloat16,
        device_map=None,                 # NEW: prevent single-GPU placement
        low_cpu_mem_usage=True           # NEW: reduce CPU peak
    )
    ...
    # CHANGED: optimizer over FSDP parameters
    decay_params = []
    nodecay_params = []
    ...
    for train_step in range(1, args.train_steps + 1):
        ...
        output_images = data_dict['output_images']
        ...
        with torch.no_grad():
            ...
            for image in output_images + input_images:
        ...
        image_seq_len = [math.prod(img_shapes_list[idx]) for idx in range(len(output_images))]
        mu = calculate_shift(
            sum(image_seq_len),
        ...
        # sample noises
        noisy_model_input_list, target_list = [], []

        for idx in range(len(output_images)):
            noises = torch.randn_like(latents_list[idx])
            noisy_model_input_list.append((1.0 - sigmas) * latents_list[idx] + sigmas * noises)
            target_list.append(noises - latents_list[idx])

        # noises = torch.randn_like(latents_list[0])
        # noisy_model_input = (1.0 - sigmas) * latents_list[0] + sigmas * noises

        packed_noisy_model_input = []
        for latents in noisy_model_input_list + latents_list[len(output_images):]:
            assert latents.shape[2] == 1
            assert latents.shape[0] == 1
        ...
        with (nullcontext() if update_grad else transformer.no_sync()):
            with torch.amp.autocast(device_type=device.type, dtype=torch.bfloat16, enabled=True):
                ...
                model_pred = torch.split(tensor=model_pred[:, :sum(image_seq_len)],
                                         split_size_or_sections=image_seq_len, dim=1)

                model_pred = [pipeline._unpack_latents(model_pred[idx], output_images[idx].height,
                                                       output_images[idx].width, pipeline.vae_scale_factor)
                              for idx in range(len(output_images))]

                loss = [F.mse_loss(model_pred[idx].float(), target_list[idx].float())
                        for idx in range(len(output_images))]

                loss = sum(loss) / len(loss)

                # model_pred = pipeline._unpack_latents(model_pred, height, width, pipeline.vae_scale_factor)
                # loss = F.mse_loss(model_pred.float(), target.float())   #  / args.accum_steps
