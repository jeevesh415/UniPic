...
class QwenImageEditPipeline(DiffusionPipeline, QwenImageLoraLoaderMixin):
    ...
    def __call__(
        ...
        # 5. Prepare timesteps
        sigmas = torch.linspace(1.0, 0.0, num_inference_steps+1, dtype=torch.float64).to(device) if sigmas is None else sigmas
        ...
        if self.scheduler.config.time_shift_type == "exponential":
            shift = math.exp(mu)
        elif self.scheduler.config.time_shift_type == "linear":
            shift = mu
        else:
        ...
        self._num_timesteps = len(timesteps) - 2
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, (t, t_next) in enumerate(zip(timesteps[:-1], timesteps[1:])):
                if self.interrupt:
                    continue
                ...
                pred_x_0 = latents - timestep * noise_pred   
                latents = (1 - t_next) * pred_x_0 + t_next * torch.randn_like(pred_x_0)   
