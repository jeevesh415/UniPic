...
if __name__ == '__main__':
    ...
    parser.add_argument('--empty_steps', default=100, type=int, help='empty steps (no training).')
    ...
    for train_step in range(1, args.train_steps+1):
        ...
        if train_step % args.student_update_freq == 0: 
            ...
            with sync_ctx:
                lora_loss = lora_loss / args.accum_steps
                lora_loss.backward()
            if sync_now:
        ...
        if update_log:
            ...
            logger.info(f"Train steps={train_step}/{args.train_steps}, Student_step={student_step}, Teacher_step={teacher_step}, "
                        f"dmd_loss={dmd_loss.item():.6f}, lora_loss={lora_loss.item():.6f}, "
                        f"grad norm={grad_norm.item():.6f}, lora grad norm={grad_norm_lora.item():.6f}, speed: {speed:.4f}, LR: {cur_lr:.8f}")
            tik = time.time()

        empty_steps = train_step % args.empty_steps == 0
        if empty_steps:
